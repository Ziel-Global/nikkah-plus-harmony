create or replace function public.privacy_visible(ps jsonb, k text)
returns boolean
language sql
immutable
set search_path = public
as $$
  select coalesce(ps ->> k, 'public') <> 'mosque_admin_only';
$$;

create or replace function public.has_active_match()
returns boolean
language sql
stable
security definer
...
as $$
  ...
  base as (
    ...
    where mp.status = 'approved'
      ...
      and (p_min_age is null or mp.date_of_birth is null or date_part('year', age(mp.date_of_birth)) >= p_min_age)
      and (p_max_age is null or mp.date_of_birth is null or date_part('year', age(mp.date_of_birth)) <= p_max_age)
      ...
      and (p_practice is null or mp.religious_practice_level = p_practice)
      and (p_languages is null or array_length(p_languages, 1) is null or mp.languages_spoken && p_languages)
...
revoke execute on function public.has_active_match() from public, anon;
revoke execute on function public.privacy_visible(jsonb, text) from public, anon;
...
grant execute on function public.has_active_match() to authenticated, service_role;
grant execute on function public.privacy_visible(jsonb, text) to authenticated, service_role;