-- Unlock hidden_until_match photos on active_match OR mutual contact consent (shared predicate)

CREATE OR REPLACE FUNCTION public.can_view_counterpart_photos(p_viewer_id uuid, p_owner_user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
    ...
    WHERE ir.status = 'active_match'
      AND (
        (ir.requester_id = p_viewer_id AND ir.target_id = p_owner_user_id)
        OR (ir.requester_id = p_owner_user_id AND ir.target_id = p_viewer_id)
      )
  )
  OR EXISTS (
    SELECT 1 FROM public.interest_requests ir
    JOIN public.contact_consents cc1 ON cc1.request_id = ir.id AND cc1.user_id = p_viewer_id
    JOIN public.contact_consents cc2 ON cc2.request_id = ir.id AND cc2.user_id = p_owner_user_id
    WHERE (ir.requester_id = p_viewer_id AND ir.target_id = p_owner_user_id)
       OR (ir.requester_id = p_owner_user_id AND ir.target_id = p_viewer_id)
  );
$$;

REVOKE EXECUTE ON FUNCTION public.can_view_counterpart_photos(uuid, uuid) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.can_view_counterpart_photos(uuid, uuid) TO authenticated, service_role;
...
CREATE POLICY "photos_select_public_approved" ON public.profile_photos
  FOR SELECT TO authenticated USING (
    ...
    OR profile_id IN (
      SELECT mp.id FROM public.marriage_profiles mp WHERE mp.user_id = auth.uid()
    )
    OR profile_id IN (
      SELECT mp.id FROM public.marriage_profiles mp
      WHERE public.can_view_counterpart_photos(auth.uid(), mp.user_id)
    )
  );
...
AS $$
  ...
  WHERE ph.photo_url = ANY(p_paths)
    AND (
      ...
      OR public.can_view_counterpart_photos(p_user_id, mp.user_id)
    );
$$;
...
as $$
  ...
  select
    ...
    (select ph.photo_url from public.profile_photos ph
      where ph.profile_id = b.id
        and (
          ph.visibility = 'public'
          or public.can_view_counterpart_photos(auth.uid(), b.user_id)
        )
    ...
    exists (
      ...
      where ph.profile_id = b.id
        ...
        and not public.can_view_counterpart_photos(auth.uid(), b.user_id)
    ),
...
grant execute on function public.browse_profiles(int,int,text,text,text,text,text,text,text[],boolean,text,text,uuid,int,int) to authenticated, service_role;
