-- Allow viewing matched member profiles across different mosques when p_profile_id is provided
...
as $$
  ...
  base as (
    ...
    where mp.status = 'approved'
      ...
      and (
        pr.mosque_id = me.mosque_id
        or (
          p_profile_id is not null 
          and exists (
            select 1 from public.interest_requests ir
            where (ir.requester_id = me.id and ir.target_id = pr.id)
               or (ir.requester_id = pr.id and ir.target_id = me.id)
          )
        )
      )
