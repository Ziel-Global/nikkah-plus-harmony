...
BEGIN
  ...
  ELSE
    ...
    INSERT INTO auth.identities (
      ...
      v_new_user_id, v_new_user_id, format('{"sub":"%s","email":"%s","email_verified":false,"phone_verified":false}', v_new_user_id::text, p_email)::jsonb, 'email', v_new_user_id::text, now(), now(), now()
    );
