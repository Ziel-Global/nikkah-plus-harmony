-- Allow users with mutual consent (or photo owners) to access non-public profile photos
...
CREATE POLICY "photos_select_public_approved" ON public.profile_photos
  FOR SELECT TO authenticated USING (
    ...
    OR profile_id IN (
      ...
      JOIN public.interest_requests ir ON (ir.requester_id = auth.uid() AND ir.target_id = mp.user_id) OR (ir.requester_id = mp.user_id AND ir.target_id = auth.uid())
      JOIN public.contact_consents cc1 ON cc1.request_id = ir.id AND cc1.user_id = auth.uid()
      JOIN public.contact_consents cc2 ON cc2.request_id = ir.id AND cc2.user_id = mp.user_id
    )
  );

-- Update browse_profiles RPC to return non-public primary photo if mutual consent exists
      ...
      where ph.profile_id = b.id 
        and (
          ...
          or exists (
            ...
            join public.contact_consents cc1 on cc1.request_id = ir.id and cc1.user_id = auth.uid()
            join public.contact_consents cc2 on cc2.request_id = ir.id and cc2.user_id = b.user_id
            where (ir.requester_id = auth.uid() and ir.target_id = b.user_id)
               or (ir.requester_id = b.user_id and ir.target_id = auth.uid())
          )
        )
