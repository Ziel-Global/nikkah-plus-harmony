...
type Row = PlatformProfile & { mosques: { name: string } | null, mosque_affiliation_requests: { status: string, mosques: { name: string } | null }[] | null };
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("profiles")
        .select(
          "id, email, phone, role, gender, mosque_id, account_status, verification_method, last_login_at, terms_accepted_at, created_at, mosques!profiles_mosque_id_fkey(name), mosque_affiliation_requests!mosque_affiliation_requests_user_id_fkey(status, mosques(name))",
        )
            ...
            >
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  {ROLE_LABEL[row.role] ?? row.role} • {row.mosques?.name ?? (() => {
                    const pending = row.mosque_affiliation_requests?.find(r => r.status === "pending");
                    if (pending) return pending.mosques?.name ? `Pending Affiliation - ${pending.mosques.name}` : "Pending Affiliation";
                    const rejected = row.mosque_affiliation_requests?.find(r => r.status === "rejected");
                    if (rejected) return "Affiliation Rejected";
                    return "Unaffiliated";
                  })()} • joined{" "}
